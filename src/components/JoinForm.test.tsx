...
describe('JoinForm', () => {
    ...
    it('does not submit if input is empty', async () => {
        render(<JoinForm />);
        fireEvent.click(screen.getByTestId('join-button'));
        await waitFor(() => expect(screen.getByTestId('join-message')).toHaveTextContent(/please enter the session code/i));
    });
    ...
    it('shows error message when code is invalid', async () => {
        ...
        render(<JoinForm />);
        fillForm('Bob', 'WXYZ');
        await waitFor(() => expect(screen.getByTestId('join-message')).toHaveTextContent(/invalid session code/i));
    });

    it('shows error if Supabase query fails', async () => {
        (supabase.from as any).mockImplementation(() => ({
            select: () => ({ eq: () => ({ single: async () => { throw new Error('Network error'); } }) }),
        }));
        render(<JoinForm />);
        fillForm('Alice', 'ABCD');
        await waitFor(() => expect(screen.getByTestId('join-message')).toHaveTextContent(/failed to join session/i));
    });

    // --- Supabase: successful join ---
    it('joins active session and navigates', async () => {
        (supabase.from as any).mockImplementation((table: string) => {
            if (table === 'sessions') return { select: () => ({ eq: () => ({ single: async () => ({ data: { id: 123, code: 'ABCD', ends_at: null }, error: null }) }) }) };
            if (table === 'restaurants') return { select: () => ({ eq: async () => ({ data: [{ id: 1, name: 'Pizza', session_id: 123 }], error: null }) }) };
            if (table === 'users') return {
                select: () => ({ eq: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) }),
                insert: (user: any) => ({ select: () => ({ single: async () => ({ data: { id: 50, ...user[0] }, error: null }) }) }),
            };
    ...
    it('joins session when user already exists without inserting new user', async () => {
        (supabase.from as any).mockImplementation((table: string) => {
            if (table === 'sessions') return { select: () => ({ eq: () => ({ single: async () => ({ data: { id: 1, code: 'ABCD', ends_at: null }, error: null }) }) }) };
            if (table === 'restaurants') return { select: () => ({ eq: async () => ({ data: [{ id: 1, name: 'Pizza', session_id: 1 }], error: null }) }) };
            if (table === 'users') return { select: () => ({ eq: () => ({ eq: () => ({ single: async () => ({ data: { id: 99, name: 'Alice', session_id: 1 }, error: null }) }) }) }) };
            return { select: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) };
        });
        render(<JoinForm />);
        fillForm('Alice', 'ABCD');
    ...
    it('shows "Session not ready yet" if session has no restaurants', async () => {
        (supabase.from as any).mockImplementation((table: string) => {
            if (table === 'sessions') return { select: () => ({ eq: () => ({ single: async () => ({ data: { id: 1, code: 'ABCD', ends_at: null }, error: null }) }) }) };
            if (table === 'restaurants') return { select: () => ({ eq: async () => ({ data: [], error: null }) }) };
            return { select: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) };
        });
        render(<JoinForm />);
        fillForm('Alice', 'ABCD');
        await waitFor(() => expect(screen.getByTestId('join-message')).toHaveTextContent(/session is not ready yet/i));
    });
    ...
    it('disables join button while submitting', async () => {
        (supabase.from as any).mockImplementation(() => ({
            select: () => ({ eq: () => ({ single: async () => ({ data: { id: 1, code: 'ABCD', ends_at: null }, error: null }) }) }),
        }));
        render(<JoinForm />);
        fillForm('Alice', 'ABCD');
    ...
    it('handles Supabase failure when fetching restaurants', async () => {
        (supabase.from as any).mockImplementation((table: string) => {
            if (table === 'sessions') return { select: () => ({ eq: () => ({ single: async () => ({ data: { id: 1, code: 'ABCD', ends_at: null }, error: null }) }) }) };
            if (table === 'restaurants') return { select: () => ({ eq: async () => { throw new Error('DB fetch failed'); } }) };
            if (table === 'users') return { select: () => ({ eq: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) }) };
            return { select: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) };
        });
    ...
    it('handles complex tie votes with last vote winning', async () => {
        ...
        (supabase.from as any).mockImplementation((table: string) => {
            if (table === 'sessions') return { select: () => ({ eq: () => ({ single: async () => ({ data: { id: 1, code: 'TIE2', ends_at: '2000-01-01T00:00:00' }, error: null }) }) }) };
            ...
            if (table === 'restaurants') return { select: () => ({ eq: () => ({ single: async () => ({ data: [{ id: 101 }, { id: 102 }, { id: 103 }], error: null }) }) }) };
            return { select: () => ({ eq: () => ({ single: async () => ({ data: null, error: null }) }) }) };
        });
