      ...
      <div className="mx-auto max-w-3xl">
        ...
        {payload && (
          <p className="text-sm text-gray-600 mt-1">
            Center ({payload.picked.lat.toFixed(4)}, {payload.picked.lng.toFixed(4)}) · radius{' '}
            {payload.radiusMi} mi ·{' '}
        ...
        {!error && !finished && havePair && (
          <div className="mt-5 rounded-2xl border p-4 shadow-sm">
            ...
            <div className="mt-3 text-xs text-gray-500">
              Compared {Math.min(cursor, Math.max(0, items.length - 1))} of{' '}
              {Math.max(0, items.length - 1)} challengers
            </div>
          </div>
        )}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-gray-600">
                      ...
                      {typeof p.rating === 'number' && (
                        <span className="ml-2">⭐ {p.rating.toFixed(1)}</span>
                      )}
                    </div>
                  </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {tag && (
          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600">{tag}</span>
        )}
      </div>
