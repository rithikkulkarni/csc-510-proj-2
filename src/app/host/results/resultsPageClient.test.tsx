...
describe('ResultsPageClient', () => {
  ...
  it(`
    ...
    expect(
      within(rank1Card).getByText(
        (_, el) => el?.tagName === 'P' && /Voted by:\s*U1,\s*U2/i.test(el.textContent || '')
      )
    ).toBeInTheDocument();
    ...
    expect(
      within(rank2Card).getByText(
        (_, el) => el?.tagName === 'P' && /Voted by:\s*U3/i.test(el.textContent || '')
      )
    ).toBeInTheDocument();
