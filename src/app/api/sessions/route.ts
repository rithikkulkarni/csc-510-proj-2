...
export async function POST(request: NextRequest) {
  ...
  const { price, price_range: priceRangeRaw, location, lat: latRaw, lng: lngRaw, radiusMiles, hours } = body || {}
  ...
  // Accept either { location: { lat, lng } } or top-level lat and lng
    ...
    function mapPriceToRange(p: any): number | null {
      ...
      if (lowered === 'moderately expensive' || lowered === 'moderate' || lowered === 'moderately') return 2
    ...
    if (price_range == null || Number.isNaN(lat) || Number.isNaN(lng) || typeof hours !== 'number') {
      return NextResponse.json({ error: 'Missing required fields: price_range (1-4), lat/lng, hours' }, { status: 400 })
    }
    ...
    if (insertErr || !insertedRow) {
      return NextResponse.json({ error: insertErr?.message || 'Failed to insert session' }, { status: 500 })
    }
    ...
    if (isDebug) {
      ...
      const debugInfo = {
        insertedRow,
        has_expiry_hours: insertedRow.expiry_hours !== undefined && insertedRow.expiry_hours !== null,
        has_ends_at: insertedRow.ends_at !== undefined && insertedRow.ends_at !== null,
      }
      return NextResponse.json({ code, id: insertedRow.id, session: insertedRow, debug: debugInfo }, { status: 201 })
    }
